...
```
Error: locator.waitFor: Error: strict mode violation: locator('div.ant-modal-confirm-btns button') resolved to 2 elements:
    1) <button type="button" class="ant-btn css-1tvv740 ant-btn-default">…</button> aka getByRole('button', { name: 'No', exact: true })
    2) <button type="button" class="ant-btn css-1tvv740 ant-btn-primary">…</button> aka getByRole('button', { name: 'Yes' })

Call log:
  - waiting for locator('div.ant-modal-confirm-btns button') to be visible

    at Calendar.addOutSiderUser (/Users/babuyadav/Documents/zillit-playwright-automation/src/pages/calendar.js:158:25)
    at Calendar.createEventForMember (/Users/babuyadav/Documents/zillit-playwright-automation/src/pages/calendar.js:188:9)
...
- dialog "Create New Event Members Personal":
  ...
  - button "Location": B 199, Central Auto Market, Block B, Sector 16, Noida, Uttar Pradesh 201301, India
  - text: Invitees
  - button "1 invitee"
  - text: "* Call Type"
  - combobox "* Call Type"
...
- dialog "Outside User":
  ...
  - text: Outside User
  - textbox "Type Email..."
  - paragraph: bhavik@gmail.com
  - button:
    - img
  - button "Submit"
- dialog:
  - img "exclamation-circle"
  - text: Add User Would you like to add the external user using this email address?
  - button "No"
  - button "Yes"
```
...
```ts
  ...
  150 |
  151 |         const emailInput = this.page.locator('[placeholder="Type Email..."]');
  152 |         await emailInput.waitFor({ state: 'visible', timeout: 10000 });
  153 |         await emailInput.fill(enterEmail);
  154 |
  155 |         await this.page.locator('div.divide-y').first().click();
  156 |
  157 |         const addButton = this.page.locator("div.ant-modal-confirm-btns button");
> 158 |         await addButton.waitFor({ state: 'visible', timeout: 5000 });
      |                         ^ Error: locator.waitFor: Error: strict mode violation: locator('div.ant-modal-confirm-btns button') resolved to 2 elements:
  159 |         await addButton.first().click();
  160 |
  161 |         const submitButton = this.page.locator('//div[@class="ant-modal-footer"]//span[text()="Submit"]');
  162 |         await submitButton.waitFor({ state: 'visible', timeout: 5000 });
  163 |         await submitButton.click();
  164 |     }
  165 |
  166 |     async fillDiscription(enterDiscription){
  167 |         await this.descriptionInput.fill(enterDiscription);
  168 |     }
  169 |
  170 |     async submitEvent(){
  171 |         await this.submit.click();
  172 |         await this.page.waitForTimeout(4000);
  173 |     }
  174 |
  175 |     async createEventForMember(){
  176 |         await this.clickCalendarTab();
  177 |         await this.clickAddEvent();
  178 |         await this.navigateToMembers();
  179 |         await this.fillEventName('Babu1');
  180 |         await this.selectStartTime();
  181 |         await this.selectEndTime();
  182 |         await this.setRepeatStatus('Every Day');
  183 |         await this.addNotification('10 Minute Before');
  184 |         await this.chooseLocation();
  185 |         await this.selectInvitees();
  186 |         await this.selectCallType('Audio Call');
  187 |         await this.chooseColor();
  188 |         await this.addOutSiderUser('Bhavik@gmail.com');
  189 |         await this.fillDiscription('This meeting for urgent');
  190 |         await this.submitEvent();
  191 |     }
  192 |
  193 |     async fullDay(){
  194 |         const fullDayToggle = this.page.locator('#fullday');
  195 |         const isChecked = await fullDayToggle.getAttribute('aria-checked');
  196 |
  197 |         if (isChecked === 'false') {
  198 |             await fullDayToggle.click();
  199 |         }
  200 |     }
  201 |
  202 |     async createEventForMemberForFullDay(){
  203 |         await this.clickCalendarTab();
  204 |         await this.clickAddEvent();
  205 |         await this.navigateToMembers();
  206 |         await this.fillEventName('Babu2');
  207 |         await this.fullDay();
  208 |         await this.selectStartTime();
  209 |         await this.setRepeatStatus('Every Day');
  210 |         await this.addNotification('One Day Before');
  211 |         await this.chooseLocation();
  212 |         await this.selectInvitees();
  213 |         await this.selectCallType('Audio Call');
  214 |         await this.chooseColor();
  215 |         await this.addOutSiderUser('Bhavik@gmail.com');
  216 |         await this.fillDiscription('This meeting for urgent');
  217 |         await this.submitEvent();
  218 |     }
  219 |
  220 |     async createEventForPersonal(){
  221 |         await this.clickCalendarTab();
  222 |         await this.clickAddEvent();
  223 |         await this.navigateToPersonal();
  224 |         await this.fillEventName('Babu3');
  225 |         await this.selectStartTime();
  226 |         await this.selectEndTime();
  227 |         await this.setRepeatStatus('Every Day');
  228 |         await this.addNotification('10 Minute Before');
  229 |         await this.chooseLocation();
  230 |         await this.chooseColor();
  231 |         await this.fillDiscription('This meeting for urgent');
  232 |         await this.submitEvent();
  233 |     }
  234 |
  235 |     async createEventForPersonalFullDay(){
  236 |         await this.clickCalendarTab();
  237 |         await this.clickAddEvent();
  238 |         await this.navigateToPersonal();
  239 |         await this.fillEventName('Babu4');
  240 |         await this.fullDay();
  241 |
  242 |         await this.selectStartTime();
  243 |         await this.setRepeatStatus('Every Day');
  244 |         await this.addNotification('One Day Before');
  245 |         await this.chooseLocation();
  246 |         await this.chooseColor();
  247 |         await this.fillDiscription('This meeting for urgent');
  248 |         await this.submitEvent();
  249 |     }
  250 |
  251 |     /**
  252 |      * Created-Event Page Actions
  253 |      */ 
  254 |
  255 |     async deleteCalenderEvent(tabName){
  256 |         await this.createdEvent.click();
  257 |
  258 |         if(tabName == 'member'){
```